...
public class EmbeddedMongoDB {
    ...
    private EmbeddedMongoDB() {
        Runtime.getRuntime().addShutdownHook(
                new Thread(this::stop, "embedded-mongodb-shutdown")
        );
    }
    ...
    public EmbeddedMongoDB withPort(int port) {
        Preconditions.checkArgument(
                ...
                "Port needs to be between 1024 and 65535"
        );
    ...
    public EmbeddedMongoDB start() {
        if (active) {
            LOG.error("Could not start EmbeddedMongoDB. Instance is already active");
            return this;
        }

        if (inUse(port)) {
            LOG.error(
                    "Could not start EmbeddedMongoDB. Port '{}' is already in use",
                    port
            );
            return this;
        }
        ...
        Mongod mongod = new Mongod() {
            ...
            public Transition<Net> net() {
                return Start.to(Net.class)
                        .initializedWith(Net.of(host, port, ipv6));
            }
        ...
        try {
            ...
            LOG.info(
                    "Successfully started EmbeddedMongoDB @ {}:{}",
                    host,
                    port
            );
        } catch (Exception e) {
            runningMongodProcess = null;
            active = false;

            LOG.error(
                    "Failed to start EmbeddedMongoDB @ {}:{}",
                    host,
                    port,
                    e
            );
    ...
    /**
     * Checks if the configured host and given port are already in use.
     *
     * @param port The port to check
     * @return True if port is in use, false otherwise
     */
    private boolean inUse(int port) {
        try (var ignored = new ServerSocket(
                port,
                0,
            ...
            LOG.warn(
                    "Port is not available for starting EmbeddedMongoDB @ {}:{}",
                    host,
                    port,
                    e
            );
